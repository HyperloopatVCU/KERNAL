...
/*FUNCTIONS*/
uint8_t readby(uint16_t);
uint16_t readw(uint16_t);

void disconnect();
void closesocket();
uint8_t opensocket();
uint8_t connect_block(uint8_t*, uint16_t);
uint8_t listen_block();

void putby(uint8_t);
uint16_t putline(uint8_t*, uint16_t);
uint8_t getby();
uint16_t getline(uint8_t*, uint16_t);

void net_config_default(socker*);
